...
const Game = () => {
  ...
  const [shuffledCountryList, setShuffledCountryList] = useState([]);
...
const showInfo = (country, code) => {
  ...
  if (clickCount >= 3) {
    showAlert('You can\'t try more than 3 times.', 5000);
  ...
  if ( shuffledIndices.length==0 && country === countryListAllIsoData[currentQuestion].name && clickCount<3) {
    if(isAnswered==false){setScore(score + 1);}
    setIsANswered(true);
    showAlertLessThan3Attempt('Correct!', country, 5000);
  } 
  else if ( shuffledIndices.length>0 && country === countryListAllIsoData[shuffledIndices[currentQuestion]].name && clickCount<3) {
    if(isAnswered==false){setScore(score + 1);}
    setIsANswered(true);
    showAlertLessThan3Attempt('Correct!', country, 5000);
  } 
  else if(clickCount<3){
    showAlertLessThan3Attempt('Wrong!', country, 5000);
  }
...
const showAlertLessThan3Attempt = (message, country, duration) => {
  const alertMessage = `${message} The country is ${country}.`;
  alert(alertMessage);
  setTimeout(() => {
    // Close the alert after the specified duration
    const alertElement = document.getElementsByClassName('alert')[0];
...
const handleNext = () => {
  ...
  useEffect(() => {
    ...
    const shuffledList = shuffleCountryList(countryListAllIsoData);
    setShuffledCountryList(shuffledList);
    console.log(shuffledCountryList[0])
  }, []);
  
  const shuffleCountryList = (countries) => {
    const shuffledList = [...countries];
  ...
  useEffect(() => {
    if (playAgainClicked) {
      const shuffledIndices = shuffleIndices(countryListAllIsoData.length);
...
//this one needs to update
  const determineFillColor = (geo) => {
    ...
    if(shuffledIndices.length>0 && geo.properties.name===countryListAllIsoData[shuffledIndices[currentQuestion]].name && stateToShow!="" && clickCount==3)
    {   
        //setScore(score+1);
        return "green";
    }

    if(shuffledIndices.length>0 && geo.properties.name===countryListAllIsoData[shuffledIndices[currentQuestion]].name && stateToShow==geo.properties.name && clickCount<3)
    {   
        //setScore(score+1);
        return "green";
    }

    if( shuffledIndices.length==0 && geo.properties.name===countryListAllIsoData[currentQuestion].name && stateToShow!="" && clickCount==3)
    {   
        //setScore(score+1);
        return "green";
    }

    if( shuffledIndices.length==0 && geo.properties.name===countryListAllIsoData[currentQuestion].name && stateToShow==geo.properties.name && clickCount<3)
    {   
        //setScore(score+1);
        return "green";
    }

    if(geo.properties.name===stateToShow && clickCount<=3)
    {   
        //setScore(score-1);
        return "red";
    }
  ...
  return (
    ...
    {showScore? (
      ...
      <div className="row">
      <div className="col">
        <p className="text-light mt-5 score-text font-weight-bold font-italic larger-font-size">
    ...
    : (
      <div className="world-map">
        ...
        <div className="text-country custom-text-country">
        ...
        {shuffledIndices.length > 0
          ? countryListAllIsoData[shuffledIndices[currentQuestion]].name
